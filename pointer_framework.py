# pointer_framework.py - Fix del bug PR #454 de lvgl_micropython
# Bug: en _calc_coords, x ya modificado se usaba para calcular y
# Fix: guardar x original antes de modificarlo
...
class PointerDriver(_indev_base.IndevBase):
    ...
    def _get_coords(self):
        ...
        # of (state, x, y) or None if no touch even has occurred
        raise NotImplementedError

    def _calc_coords(self, x, y):
        if self.is_calibrated:
            cal = self._cal
            # FIX PR #454: guardar x original antes de modificarlo
            # para que el calculo de y use el x original, no el ya transformado
            x_orig = x
            y_orig = y
