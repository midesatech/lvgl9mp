...
# NO borrar calibracion - usar la guardada
...
lbl = lv.label(scrn)
lbl.set_text("Toca esquinas:\nTOP-LEFT, TOP-RIGHT\nBOT-LEFT, BOT-RIGHT\nCENTER")
lbl.set_style_text_color(lv.color_hex(0xFFFFFF), 0)
lbl.align(lv.ALIGN.TOP_MID, 0, 5)

coords_lbl = lv.label(scrn)
coords_lbl.set_text("x=? y=?")
coords_lbl.set_style_text_color(lv.color_hex(0x00FF00), 0)
coords_lbl.align(lv.ALIGN.BOTTOM_MID, 0, -5)

def screen_cb(e):
    p = lv.point_t()
    indev.get_point(p)
    coords_lbl.set_text("x={} y={}".format(p.x, p.y))
